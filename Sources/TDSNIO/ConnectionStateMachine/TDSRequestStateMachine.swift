...
struct TDSRequestStateMachine {
    ...
    private var rowStreamStateMachine: TDSRowStreamStateMachine?
    ...
    private static func action(from action: TDSRowStreamStateMachine.Action) -> Action {
        switch action {
        case .read:
            return .read
        case .wait:
